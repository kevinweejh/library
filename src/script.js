...
const removeFromLibrary = (bookIndex) => {
    if (myLibrary.length == 0) {
        console.log("initial removal")
        if (bookIndex == 0) {
            myLibrary = myLibrary.shift();
        } else {
            myLibrary = myLibrary.slice(0, bookIndex).concat(myLibrary.slice(bookIndex + 1))
        }
    } else {
        console.log("subsequent removal");
        myLibrary = myLibrary.slice(0, bookIndex).concat(myLibrary.slice(bookIndex + 1));
    }
...
const updateBookShelf = (myLibrary = myLibrary) => {
    bookShelf.innerHTML = '';
    let bookCount;
    ...
    myLibrary.forEach((book) => {
        ...
        bookControls.setAttribute("id", bookCount);
        bookControls.innerHTML = `<button id="removeBtn" class="absolute top-5" onclick="removeFromLibrary(${bookCount})"><svg class="svg-icon" viewBox="0 0 20 20">
    ...
    </svg></button><button id="readBtn" class="absolute" onclick="toggleBookRead(${bookCount})"><svg class="svg-icon" viewBox="0 0 20 20">
